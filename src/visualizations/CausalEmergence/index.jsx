...
export default function CausalEmergenceCalculator() {
  ...
  const animationCallback = useCallback(() => {
    ...
    const microCenterX = W * 0.25;
    const microCenterY = H * 0.45;
    const microRadius = Math.min(W * 0.18, H * 0.35);
      ...
      for (let to = 0; to < n; to++) {
        ...
        if (prob > 0.05) {
          ...
          if (from === to) {
            ...
            ctx.strokeStyle = `rgba(245, 158, 11, ${prob * 0.6})`;
            ...
            ctx.strokeStyle = `rgba(245, 158, 11, ${prob * 0.5})`;
            ctx.lineWidth = 1 + prob * 2;
            ctx.stroke();
    ...
    const macroCenterX = W * 0.75;
    const macroCenterY = H * 0.45;
    const macroRadius = Math.min(W * 0.12, H * 0.25);
      ...
      for (let to = 0; to < nMacro; to++) {
        ...
        if (prob > 0.05) {
          ...
          if (from === to) {
            ...
            ctx.beginPath();
            ctx.arc(cx, cy, loopRadius, 0, Math.PI * 2);
            ctx.strokeStyle = `rgba(34, 197, 94, ${prob * 0.6})`;
            ctx.lineWidth = 2 + prob * 3;
            ctx.stroke();
          } else {
            const dx = toPos.x - fromPos.x;
            const dy = toPos.y - fromPos.y;
    ...
    ctx.font = 'bold 12px Inter, system-ui';
    ctx.textAlign = 'center';
    ctx.fillText(`EI = ${formatNumber(microMetrics.ei)}`, W * 0.25, barY + barHeight + 18);
    ...
    ctx.fillStyle = '#22c55e';
    ctx.fillText(`EI = ${formatNumber(macroMetrics.ei)}`, W * 0.75, barY + barHeight + 18);
